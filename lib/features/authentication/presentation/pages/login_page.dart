    ...
    return BlocListener<AuthenticationBloc, AuthState>(
      ...
      listener: (context, state) {
        if (state is AuthStateLoginSuccess) {
          context.go(RouteConstants.home, extra: {'replaceAll': true});
        } else {
          CustomToast.show(context, 'login failed!', isError: true);
        }
      },
      listenWhen: (_, curr) =>
          curr is AuthStateLoginSuccess || curr is AuthStateLoginFailed,
      child: Scaffold(
        appBar: AppBar(
          leading: BackButton(
            onPressed: () {
