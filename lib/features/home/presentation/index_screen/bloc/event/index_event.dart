...
class IndexEvent with _$IndexEvent {
  ...
  /// LoadRecentTaskEvent is an event that will be used to
  /// load recent tasks.
  const factory IndexEvent.loadRecentTasks() = LoadRecentTasksEvent;

  /// MarkDoneEvent is an event that will be used to
  /// mark a task as done.
  const factory IndexEvent.navigateToTaskDetails(
    TaskModel task,
  ) = NavigateToTaskDetailsEvent;

  /// ChangeDaysFilterEvent is an event that will be used to
  /// change the days filter.
  const factory IndexEvent.changeDaysFilter(
  ...
  const factory IndexEvent.changeStatusFilter(
    TaskStatus status,
  ...
  /// this event is added when ever we are getting notified that
  /// there is a task that got created/updated/deleted
  const factory IndexEvent.taskTableEvent(
    TaskTableEvents taskEvent,
  ) = TaskTableEvent;
}
